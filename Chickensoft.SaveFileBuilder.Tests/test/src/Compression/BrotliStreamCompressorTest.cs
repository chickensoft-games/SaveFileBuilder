...
{
  private readonly BrotliStreamCompressor _compressor;

  public BrotliStreamCompressorTest()
  {
    _compressor = new BrotliStreamCompressor();
  }

  [Fact]
  public void Compress_WithDefaultParameters_ReturnsBrotliStream()
  {
  ...
  [Fact]
  public void Compress_WithOptimalCompression_ReturnsBrotliStream()
  {
    using var baseStream = new MemoryStream();
  ...
  [Fact]
  public void Compress_WithFastestCompression_ReturnsBrotliStream()
  {
    using var baseStream = new MemoryStream();
  ...
  [Fact]
  public void Compress_WithSmallestSizeCompression_ReturnsBrotliStream()
  {
    using var baseStream = new MemoryStream();
  ...
  [Fact]
  public void Compress_WithNoCompression_ReturnsBrotliStream()
  {
    using var baseStream = new MemoryStream();
  ...
  [Fact]
  public void Decompress_WithValidStream_ReturnsBrotliStream()
  {
    // Create a compressed stream first
    using var compressedData = new MemoryStream();
    using (var brotliStream = new BrotliStream(compressedData, CompressionMode.Compress, leaveOpen: true))
    {
  ...
  public void CompressAndDecompress_RoundTrip_PreservesData()
  {
    ...
    using (var brotliCompress = _compressor.Compress(compressedStream, leaveOpen: true))
    {
      brotliCompress.Write(originalBytes, 0, originalBytes.Length);
    }

    // Decompress
    compressedStream.Position = 0;
    using var decompressedStream = new MemoryStream();
    using (var brotliDecompress = _compressor.Decompress(compressedStream, leaveOpen: true))
    {
      brotliDecompress.CopyTo(decompressedStream);
    }
  ...
  public void CompressAndDecompress_WithLargeData_PreservesData()
  {
    ...
    // Compress
    using var compressedStream = new MemoryStream();
    using (var brotliCompress = _compressor.Compress(compressedStream, CompressionLevel.Optimal, leaveOpen: true))
    {
      brotliCompress.Write(originalBytes, 0, originalBytes.Length);
    }
  ...
  public void Compress_WithDifferentCompressionLevels_ProducesDifferentSizes()
  {
    ...
    using (var brotliFastest = _compressor.Compress(fastestStream, CompressionLevel.Fastest, leaveOpen: true))
    {
      brotliFastest.Write(testBytes, 0, testBytes.Length);
    }

    // Compress with SmallestSize
    using var smallestStream = new MemoryStream();
    using (var brotliSmallest = _compressor.Compress(smallestStream, CompressionLevel.SmallestSize, leaveOpen: true))
    {
      brotliSmallest.Write(testBytes, 0, testBytes.Length);
    }
